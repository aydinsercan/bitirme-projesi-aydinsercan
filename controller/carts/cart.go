...
import (
	...
	CartModel "github.com/aydinsercan/basketapi/model/cart"
)
...
func (r *Router) Order(c echo.Context) error {
	mod := CartModel.Cart{}
	err := mod.Order()
	if err != nil {
		return r.Send(c, http.StatusInternalServerError, "Cannot flush data", err.Error(), nil)
	}
	return r.Send(c, http.StatusOK, "Order successfully placed", "", nil)
}
